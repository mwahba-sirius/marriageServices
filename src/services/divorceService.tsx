...
import { DivorceData } from "../steps/divorceData";
...
export const DivorceService = () => {
    ...
    const divorceRef = useRef<IStepRef | null>(null);
    ...
    return (
        <div style={{ width: "100%", display: "flex", justifyContent: "center", marginTop: "2rem", alignItems: "stretch" }}>
            <Stepper serviceName="طلب توثيق عقد طلاق" steps={[
                {
                ...
                {
                    element: () => <DivorceData ref={divorceRef} defaultValues={marriageSlice.divorce} />, stepName: "بيانات الطلاق", onNext: async () => {
                        return (await divorceRef.current?.onNext((data) => dispatch(marriageSliceActions.setDivorceData(data))) ?? true)
                    }
                },
                {
